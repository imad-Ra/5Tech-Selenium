...
class TestAPIFlightStatus(unittest.TestCase):
    ...
    def test_airports_supporting_data_feed(self):
        ...
        """
        response = self.api_request.get_airport_data_feed_services_status()
        flight_status_nearest = response.json()
        print(flight_status_nearest)

        self.assertTrue(response.ok)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(flight_status_nearest["flightSchedulesFeed"]["service"], self.config["service"]["0"])
