...
class APIFlightApi:
    ...
    def get_flight_status_by_date(self,querystring):
        url = f"{self.url}{self.flights_number}/{self.config["flight-numbers"]["1"]}/{self.config["time"]["date_of_flight_status_by_date"]}"
        return self._request.get_request(url, headers=self.headers, params=querystring)
    ...
    def get_fids_schedules_airport_departures_time_range(self, querystring):
        url = f"{self.url}{self.flights_airports}{self.iata_code_type}/{self.config["airports-codes"]["1"]}/{self.config["time"]["fids_schedules_time_range"]}"
        return self._request.get_request(url, headers=self.headers, params=querystring)

    def get_fids_schedules_airport_departures_relative_time(self, querystring):
